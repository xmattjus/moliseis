...
void main() {
  group('RelationUpdate synchronization semantics', () {
    test('Keep should preserve existing relation targetId', () {
      final entity = FakeEntity();

      applyRelationUpdate(const Keep(), entity.relation);

      expect(entity.relation.targetId, equals(100));
    });

    test('Clear should remove existing relation', () {
      final entity = FakeEntity();

      applyRelationUpdate(const Clear(), entity.relation);

      expect(entity.relation.targetId, equals(0));
    });

    test('Assign should update relation targetId', () {
      final entity = FakeEntity();

      applyRelationUpdate(const Assign(42), entity.relation);

      expect(entity.relation.targetId, equals(42));
    });

    test('Assign should replace previous relation value', () {
      final entity = FakeEntity();

      entity.relation.targetId = 12;

      applyRelationUpdate(const Assign(99), entity.relation);

      expect(entity.relation.targetId, equals(99));
    });

    test('Clear should remove previously assigned relation', () {
      final entity = FakeEntity();
  ...
  group('backend payload semantics', () {
    test('Keep should represent omitted backend field', () {
    ...
    test('Clear should represent explicit backend null', () {
    ...
    test('Assign should represent explicit backend value', () {
  ...
  group('partial synchronization behavior', () {
    test('multiple updates should preserve untouched relations', () {
      final projectRelation = FakeToOneRelation(10);
      final ownerRelation = FakeToOneRelation(20);

      applyRelationUpdate(const Keep(), projectRelation);

      applyRelationUpdate(const Assign(50), ownerRelation);

      expect(projectRelation.targetId, equals(10));
      expect(ownerRelation.targetId, equals(50));
    });

    test('Clear should be distinguishable from Keep', () {
      final keepRelation = FakeToOneRelation(77);
      final clearRelation = FakeToOneRelation(77);

      applyRelationUpdate(const Keep(), keepRelation);

      applyRelationUpdate(const Clear(), clearRelation);
  ...
  group('pattern matching exhaustiveness', () {
    test(
      ...
      () {
        const updates = <RelationUpdate<int>>[Keep(), Clear(), Assign(5)];
        ...
        expect(results, equals(['keep', 'clear', 'assign:5']));
      },
    );
  });
}
