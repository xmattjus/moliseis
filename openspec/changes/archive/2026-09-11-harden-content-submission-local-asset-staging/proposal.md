...
- Require the current draft identity to be known durable before opening `ImagePicker.pickMultipleMedia()`, using the persisted-draft `unknown`/`absent`/`restored` state as the authority so a fresh empty draft is physically persisted once.
...
- Treat ViewModel disposal as terminal for runtime publication while allowing already-started durable repository work to finish for the next initialization.
- Persist single-asset removal, provide idempotent per-session cleanup, and remove temporary, stale-descriptor, descriptor-less-final, and non-active orphan state through deterministic reconciliation.
- Keep repository validation at acquisition and reconciliation trust boundaries, resolve already-trusted staged paths structurally, and expose only the support-directory environment boundary rather than internal filesystem-operation hooks.
...
- Adds a minimal local staged-asset domain/data model, ObjectBox entity and generated metadata, Content Submission-specific repository implementation, private filesystem storage, Provider registration, and a replaceable support-directory boundary for real-filesystem tests.
