...
- Guard removal of the Content Submission form route: clean sessions exit directly, while dirty sessions offer exactly `Salva ed esci`, `Esci senza salvare`, and `Annulla` with explicit checkpoint, checkpoint-revert, and cancellation semantics; dirty means only uncheckpointed `ContentSubmissionDraft` field state, never immediately durable staged-asset membership. Only the owning exit invocation may receive affirmative permission, so overlapping removals cannot consume the same decision and remove another route.
- Prevent a native iOS edge-swipe from starting while the form is dirty or a form-owned transition boundary is pending, because an asynchronous route veto entered after an interactive Cupertino pop has started can leave the route visually frozen. A dirty AppBar Back uses a one-shot initiation allowance that is revoked immediately after it starts its programmatic pop; it never remains active for the asynchronous exit decision. Keep the existing AppBar Back exit policy, retain clean idle native swipe-back, and show a secondary `● Modifiche non salvate` status line within the unchanged standard AppBar geometry.
- Add the minimum ViewModel lifecycle operation needed to restore only uncheckpointed `ContentSubmissionDraft` fields to the last checkpoint while preserving the persisted draft, session identity, and current already-durable staged-asset membership; it does not implement asset undo or asset history.
