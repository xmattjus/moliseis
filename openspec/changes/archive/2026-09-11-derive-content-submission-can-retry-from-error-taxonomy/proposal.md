...
- Backend and rollout: no Edge Function, SQL, RPC, migration, RLS, idempotency, deployment negotiation, or release operation is added. The released client's immediate-retry policy is deliberately safe across both the legacy and hardened server generations during the planned client-first rollout.
