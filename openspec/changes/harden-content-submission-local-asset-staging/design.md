...
Create one memoized initialization future owned by the ViewModel. Its body distinguishes three draft-read outcomes: restored draft → reconcile/load that identity; `Success(null)` → null-active orphan cleanup; `Error` → retain unknown ownership, skip reconciliation, and expose no runtime assets. Only the first two outcomes invoke staged reconciliation. It reconstructs `_assets` from resolved final paths when trustworthy, and only then completes the barrier, sets `loadState` ready, and notifies. Repeated `initialize()` calls return the same future and perform no second load/reconciliation. A valid loaded draft establishes both `_hasDurableCheckpoint = true` and the recovery-eligible ID. `Success(null)` and draft-load failure keep the fresh ID non-durable, but only the former authorizes orphan cleanup; neither supplies recovery ownership. Every local session mutation/checkpoint awaits this future before arbitration, eliminating late-load overwrite races.
