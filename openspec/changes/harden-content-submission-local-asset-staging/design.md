...
### 2. Use persisted-draft state as the sole durability authority

Retain one small private persisted-draft status with `unknown`, `absent`, and `restored` values. This is the complete durable-existence contract rather than being paired with a second boolean. `unknown` means a persisted draft may exist but could not be read, so checkpointing, picker staging, and orphan cleanup are forbidden. `absent` means durable storage authoritatively contains no draft and the first checkpoint for the current in-memory identity must perform a real save. `restored` means the current identity was either loaded or successfully saved and unchanged checkpoint equality may skip another write. Successful clear returns the state to `absent` for the fresh identity.

`Success(null)` permits null-active reconciliation/orphan cleanup. A load error leaves state `unknown`, retains its error for `Result` propagation, skips staged reconciliation entirely, blocks fresh-draft checkpointing/picking, and never adopts prior staged state. A successful explicit `clearDraft()` establishes `absent`; when clearing from `unknown`, it immediately permits null-active global orphan cleanup before the fresh identity is exposed.
...
Disposal is a terminal runtime-publication boundary, not cancellation of already-started durable work. Check it before a new user-visible operation and immediately before runtime list/state publication or notification. Do not branch after each harmless size, digest, repository, or path operation. A repository commit may complete after disposal and remain durable for next initialization; do not roll it back merely because runtime publication is skipped.
...
Alternative considered: random final filenames plus stored source names. Rejected because deterministic digest names simplify idempotent retry while source names add unneeded metadata.
...
The repository also converges retries by querying `(clientSubmissionId, digest)` and using the deterministic final path. If an equivalent descriptor/final pair already exists, it validates and returns the existing descriptor rather than adding another. A final file without a descriptor is uncommitted orphan state and is replaced by a fresh verified acquisition or removed during reconciliation; it is never adopted. The ViewModel's digest list preserves current soft duplicate semantics and avoids unnecessary copies.
...
Never persist a temp path. A copy/rename failure removes its own temp on a best-effort basis and returns `Result.error`. A descriptor failure after final rename returns error and may leave an unreferenced final file, which retry may replace and reconciliation deletes as orphan state. Earlier candidates already committed in the batch remain committed. This intentionally accepts loss of the one uncommitted attachment in the narrow final-rename-to-descriptor window, while preserving the draft and every previously committed attachment, without a journal or all-or-nothing transaction.
...
- For each active descriptor in ID order, perform one complete validation of its expected regular final path, size, and digest. If final is missing or invalid, delete the stale descriptor and omit the broken/untrusted file from restoration.
...
- Scan immediate active-directory entries once and delete every temporary or descriptor-less final file as orphan/incomplete state. Do not hash descriptor-less finals, allocate reconstruction IDs, or re-query merely to order reconstructed rows.
...
- When `loadDraft()` successfully returns `null`, restore nothing and clean every staged descriptor/session directory as orphan state. When draft loading fails, do not call reconciliation at all: preserve descriptors/files because persisted ownership is unknown, expose no runtime assets, and never adopt an old directory into the ViewModel's newly generated draft ID.

The repository may log path-free diagnostics for stale/malformed cleanup. Reconciliation treats expected inconsistency as repair, not a fatal error. True ObjectBox/filesystem failures return `Result.error`. Initialization still waits for that result before its shared future completes; on a hard reconciliation failure it exposes no partially restored assets, stores the error, marks staged state unavailable, and transitions the existing two-state UI to ready rather than adding error UX. Recovery then drops its response best-effort. Before a later normal add opens the picker, its serialized pre-picker phase retries reconciliation for the now-durable active ID and returns the reconciliation error without picker invocation if state remains unavailable. Clear remains available through draft-first, best-effort old-session cleanup; removal cannot be offered because no runtime assets were restored. This is the only retry behavior—do not leave it optional or invent another initialization state.
...
Accepted trade-off: process termination after final rename but before descriptor persistence can lose that one not-yet-committed attachment. Deleting the unreferenced final on reconciliation keeps the normal flow understandable and leaves the persisted draft and all committed descriptor-backed attachments recoverable. This is preferred over permanent reconstruction machinery for a very narrow window.
...
### 10. Make initialization and lost-data processing share one memoized future

Create one memoized initialization future owned by the ViewModel and use it directly as both the single-flight primitive and completion contract. Its body distinguishes three draft-read outcomes: restored draft → reconcile/load that identity; `Success(null)` → null-active orphan cleanup; `Error` → retain unknown ownership, skip reconciliation, and expose no runtime assets. Only the first two outcomes invoke staged reconciliation. It reconstructs `_assets` from already validated descriptors and structurally resolved final paths when trustworthy, then sets `loadState` ready and notifies before completing. Repeated `initialize()` calls return the same future and perform no second load/reconciliation. A valid loaded draft establishes `restored` state and the recovery-eligible ID. `Success(null)` establishes `absent`; draft-load failure keeps `unknown`; neither supplies recovery ownership. Every local session mutation/checkpoint and deferred recovery processing awaits this future directly, eliminating late-load overwrite races without a separate `Completer`.

`_retrieveLostAssets()` retains current platform guards and calls `ImagePicker.retrieveLostData()` immediately when Android triggers it. It stores the returned response in that async invocation, then awaits the initialization future. Initialization records a recovery-eligible identity only when a valid persisted draft and trustworthy staged state were restored. Recovery captures that identity after the future completes, enters lifecycle arbitration, and compares it with the current active ID before touching candidates; clear rotation therefore makes the response stale. Only on equality does it prioritize non-null `response.files` (without retaining the current `response.file != null` gate), fall back to the single `response.file` when needed, and route the files through the common candidate routine. Capacity and duplicate checks therefore see restored assets.

The Android platform call remains before waiting for initialization, including when it throws. Every Android recovery exit starts and awaits the shared initialization future after that attempt, so a direct recovery call cannot leave draft loading, orphan reconciliation, or `loadState` pending. A thrown or response-level plugin error remains a non-actionable recovery success after initialization completes.

If no valid persisted draft was restored, or reconciliation did not establish trustworthy active staged state, recovered files are logged/dropped best-effort and never attached to the fresh ID. A serialized candidate-pipeline `Result.error` is likewise recorded through the recovery error handler after processing, without rethrowing or changing Command UX. Empty responses and plugin errors remain non-actionable successes and never delete valid staged assets. `ContentSubmissionScreen.initState()` may keep its existing early trigger; correctness no longer depends on screen timing.
...
`removeAssetAt` synchronously captures the selected runtime asset's digest and the current `clientSubmissionId` before waiting for lifecycle arbitration. Inside the boundary it compares the captured ID with the active ID and confirms that the captured digest is still present; a mismatch is stale work and completes without touching the new session. It then asks the repository to remove that identity+digest and removes the matching runtime entry (not whichever item later occupies the original index) before notifying on success. The repository removes the descriptor first and treats a missing file as success. If deleting an existing file fails after descriptor removal, it returns an error and leaves runtime state unchanged; retry can derive and remove the same deterministic path even without a descriptor, while process-restart reconciliation treats the retained unreferenced final as orphan state. This favors recoverability over pretending the two deletions are atomic.

For full discard, keep Subplan 1's hard boundary: call `draftRepository.clearDraft()` before destructive staged cleanup. If draft clear fails, do not clean staged state or rotate. After it succeeds, clear the old active session while still under arbitration and before exposing a new identity. When the pre-clear persisted state was `unknown`, the active identity is only a fresh constructor identity, so use null-active reconciliation to clean all quarantined staged state as authoritative orphans instead of `clearSession(oldId)`. Because the draft is already absent, a cleanup failure cannot safely preserve the old session as active; finish rotation, return discard success, and leave residue foreign to the new ID so the next reconciliation retries orphan cleanup. The staged repository, not the ViewModel, emits exactly one path-free cleanup diagnostic containing only safe operation/error-category metadata; it must not forward raw filesystem exceptions or stack traces, including from null-active post-clear reconciliation. This ordering avoids the worse alternative where assets are deleted but a failed draft clear leaves a supposedly recoverable draft with missing attachments.
...
Alternative considered: deletion tombstones. Rejected because deterministic paths, idempotent retry, and orphan reconciliation cover the required partial failures.
...
Repository and reconciliation tests use `test/support/objectbox_test_store.dart` with a real temporary ObjectBox Store plus a separate temporary staging root supplied through `getSupportDirectory`. They create missing files, malformed descriptors, duplicate rows, orphan/temp files, and symlink entries through real state rather than production operation overrides. ViewModel tests extend shared fakes/gates in `test/support` for interface-level lifecycle ordering. Retain races that protect realistic cross-Command identity behavior, but do not preserve exact intra-filesystem-operation TOCTOU or disposal-between-every-await tests.

## Risks / Trade-offs

- [A fresh equal draft skips its first physical save] → Permit equality-based skipping only in persisted-draft `restored` state and test the initial absent draft, loaded draft, successful save, failed save, and post-clear identity independently.
...
- [Process death occurs between final rename and descriptor write] → Accept loss of that one uncommitted attachment and delete the unreferenced final as orphan state on reconciliation; preserve the draft and committed attachments.
- [Picker source changes after candidate digest] → Hash the completed temporary copy and require equality before final rename.
- [Picker source grows after the first size check] → Recheck the completed temporary file length before final rename.
...
- [Draft load failure is confused with authoritative absence] → Retain the explicit `unknown` persisted-draft status, skip null-active reconciliation and fresh-draft writes while it remains unknown, and test descriptor/file preservation, blocked picker/checkpoint, recovery drop, and the separate `Success(null)` cleanup path.
- [Successful explicit clear leaves unknown-session assets behind] → Capture persisted state before draft-first clear; after a successful clear from `unknown`, use null-active orphan reconciliation before rotation, while a clear error preserves every quarantined descriptor/file.
...
- [Disposed ViewModel publishes post-async asset changes] → Check disposal at operation entry and runtime publication/notification boundaries; allow already-started durable repository work to complete for next initialization.
- [Clear rotates after lost-data restoration but before processing] → Capture the recovery-eligible restored ID after initialization completes and compare it inside lifecycle arbitration.
...
- [Cleanup diagnostics leak private filesystem paths] → Have the staged repository log one safe cleanup event without raw filesystem errors or stack traces; test a recognizable private path is absent from every logger payload.
- [Recovered candidate processing returns an error without throwing] → Inspect the serialized `Result`, record it through the recovery diagnostic path, and keep recovery non-blocking with valid staged assets unchanged.
- [Removal partially deletes descriptor/file] → Derive file location from validated identity+digest, keep runtime state on hard failure, allow idempotent retry, and let reconciliation clean a retained unreferenced final file as orphan state.
...
3. Prove repository staging, deterministic naming, relative-path containment, explicit descriptor ordering, reconciliation, removal, session cleanup, and orphan cleanup against real temporary storage without per-operation production hooks.
4. Use persisted-draft state as the durability authority and retain the private lifecycle queue; split add around the external picker, reject stale session results, and route normal candidates through durable staging.
5. Coordinate initialization/restoration and early Android lost-data acquisition with one memoized future, then persist removal and draft-first clear/session cleanup.
