...
Subplans 1 and 2 are implemented and complete. `ContentSubmissionViewModel` already owns a stable `clientSubmissionId`, immutable `_checkpointedDraft`, derived `hasUnsavedChanges`, awaitable `checkpointDraft()`, memoized initialization, `_PersistedDraftState`, one private lifecycle queue, durable staged assets, retry-safe staged upload sources, and persistence-first `clear`. `hasUnsavedChanges` is exactly the uncheckpointed `ContentSubmissionDraft` field comparison (`_state != _checkpointedDraft`); durable staged-asset additions, removals, and reconciliation are deliberately separate and never participate in it. Asset picking already checkpoints inside the ViewModel before invoking `ImagePicker`; that ownership remains unchanged. The root Provider creates this ViewModel once for the application, so route removal does not dispose or reset its state.

Before this change, the form route had no exit policy. Its Terms and Privacy handlers launched through `UrlLaunchService` without checkpointing. Its submit handler validated, started `submit.execute()` unawaited, and independently pushed the child progress route, so Command re-entry protection did not prevent duplicate pushes. The progress screen used `PopScope(canPop: !submit.running)` for system and predictive back, but owned completed-session cleanup in three navigation paths and cleared even failed work when `Torna alla home` was used.

The project resolves `go_router` 17.3.0. Its installed source defines `GoRoute.onExit` as `FutureOr<bool> Function(BuildContext, GoRouterState)`, calls it when a route is removed by pop or a new route configuration such as `go`, and evaluates exiting matches from child to parent. The pop path schedules each asynchronous callback independently and calls `_completeRouteMatch` for each invocation that resolves true; concurrent pop attempts therefore must not share one affirmative Future. An `onExit` on the existing parent form route still covers removal of that form by AppBar/system back and programmatic replacement while allowing a pop of only the progress child without invoking the parent policy. The route-local ownership gate rejects competing removal while a form checkpoint boundary is pending; the form `PopScope` is separately required only for native iOS interactive-pop disposition.
...
- Keep the production delta within the justified 100–300 LOC post-iOS range through localized additions and deletion of current progress cleanup coordination. A 250-line threshold triggers mandatory simplification and review; it is not a final implementation ceiling.
...
- Do not add asset-level dirty tracking, checkpoint snapshots, membership history, undo/redo, or transactional draft-plus-asset rollback.
...
On Flutter 3.41.9, the form adds one narrowly scoped iOS `PopScope` disposition so Cupertino's `popGestureEnabled` rejects the native edge swipe before its transition begins while the form is dirty or a form-owned boundary remains pending. This `PopScope` owns no persistence policy, dialog, restore, clear, or route-removal decision: `GoRoute.onExit` remains the sole dirty/clean removal policy. A dirty explicit-AppBar-Back allowance opens only long enough to initiate its one programmatic `context.pop()` after the rebuilt frame, then is revoked in the immediately following frame regardless of the asynchronous `GoRoute.onExit` decision. It does not remain active while a dialog, checkpoint, restore, cancellation, or failure is pending, and `onPopInvokedWithResult` is not relied upon as its only revocation path. Android remains outside this scope and retains its route-owned system/predictive policy. The progress route retains its separate progress-specific `PopScope`. The form AppBar derives a secondary `● Modifiche non salvate` line directly from `hasUnsavedChanges` inside the existing standard toolbar without changing any AppBar dimensions or Back control.
...
This is sufficient because Subplan 1 already retains the exact immutable checkpointed `ContentSubmissionDraft` field snapshot and Subplan 2 intentionally persists staged assets through separate immediately durable operations. `Esci senza salvare` restores only that draft snapshot: it does not undo a durable addition, resurrect a durable removal, restore historical asset membership, or clear current staged assets. Re-reading the draft repository would add I/O and a second restoration path without improving correctness.
...
The existing progress `PopScope` remains the separate progress-specific widget-level pop policy. Its `canPop` expression expands minimally to include local-finalization-pending. The form's narrowly scoped iOS `PopScope` remains only a native interactive-pop disposition and does not participate in progress policy.
...
**Production-code delta:** `552` additions and `276` deletions, for a net `+276 LOC` against `b69c81c`, within the approved `100–300 LOC` range. Progress cleanup deletion offsets route/form/ViewModel additions; tests and OpenSpec documentation are excluded. The former 250-line ceiling is exceeded only by the Flutter 3.41.9-required iOS route pop disposition, one-shot explicit-Back initiation/revocation, and accessible unchanged-height dirty-status composition needed to prevent an already-started native gesture from freezing on dialog cancellation. These remain localized to the existing form widget and add no production type, file, service, or navigation participant.
...
- [An async route veto starts after a Cupertino interactive pop] → Prevent iOS edge-swipe initiation while dirty or while a form-owned transition boundary remains pending through the form pop disposition. The first revised iOS Simulator 26.5 matrix found that retaining the AppBar allowance until a denied asynchronous pop callback leaked native swipe eligibility after `Annulla`; revoke the one-shot allowance immediately after the explicit pop initiation instead, retain the normal AppBar-triggered `onExit` policy, and rerun the complete native iOS matrix.
...
- [Production growth reaches the 250-line review threshold] → Run `git diff --numstat -- lib/routing/router.dart lib/ui/content_submission/view_models/content_submission_view_model.dart lib/ui/content_submission/widgets/content_submission_screen.dart lib/ui/content_submission/widgets/content_submission_progress_screen.dart` and calculate production additions minus deletions, excluding tests/docs/generated output. Simplify duplicated route/widget/progress branching where possible, then record both gross and net figures plus block-by-block justification for the already approved 100–300 LOC post-iOS range before handoff.
