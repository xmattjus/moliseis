...
class EventsMonth extends StatefulWidget {
  ...
  final List<EventContent?> displayedMonthEvents;

  final ValueChanged<EventContent?> onDaySelected;
...
class _EventsMonthState extends State<EventsMonth> {
  ...
  List<Widget> _dayHeaders(
    ...
    for (
      ...
      final String weekday = _dateSymbols.SHORTWEEKDAYS[i];
      result.add(
        ExcludeSemantics(
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final event = widget.displayedMonthEvents.firstWhere(
          (event) => event?.startDate.day == day,
          orElse: () => null,
        );
  ...
  Widget build(BuildContext context) {
    ...
    */
    ...
    final Color? dayForegroundColor = resolve<Color?>(
      (DatePickerThemeData? theme) => !widget.isDisabled
    ...
    if (widget.isDisabled) {
      ...
      dayWidget = InkResponse(
        ...
        child: Semantics(
          ...
          label:
              '${localizations.formatDecimal(widget.day.day)}, ${widget.calendarDelegate.formatFullDate(widget.day, localizations)}',
